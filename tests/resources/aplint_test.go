...
// templatePlaceholderRe matches an unsubstituted template token such as
// {name} or {namespace} — a flow scalar of the shape {identifier} with no
// colon, which never appears in a concrete, applied resource. Fixtures
// carrying these are rendered at test runtime via string substitution and
// cannot be strict-parsed as-is, so the fixture-directory scan skips them.
// The linter itself (LintApplicationProfileYAML) stays strict for real
// callers like `bobctl lint`.
...
func TestApplicationProfileFixturesLint(t *testing.T) {
	...
	for _, p := range matches {
		t.Run(filepath.Base(p), func(t *testing.T) {
			...
			violations := LintApplicationProfileYAML(data, p)
			// A template fixture (e.g. metadata.name: {name}) fails strict
			// parsing with a lone R-AP-00. When that parse failure is caused
			// by an unsubstituted {placeholder}, skip — it's rendered at
			// runtime, not a concrete AP. A genuine parse failure (no
			// placeholder) is still surfaced, and a doc that merely contains
			// {...} elsewhere but parses cleanly is linted normally.
			if len(violations) == 1 && violations[0].Rule == "R-AP-00" && templatePlaceholderRe.Match(data) {
				t.Skipf("template fixture (unsubstituted placeholder)")
			}
