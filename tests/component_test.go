...
func Test_27_ApplicationProfileOpens(t *testing.T) {
	...
	t.Run("recorded_profile_absolute_paths", func(t *testing.T) {
		...
		require.NoError(t, err)
		dyn := k8sinterface.NewKubernetesApi()
		storageClient := spdxv1beta1client.NewForConfigOrDie(dyn.K8SConfig)
		...
		detail := ""
		if !passed {
			detail = "found non-absolute, '.', or scrambled paths in recorded profiles"
		}
		addResult("recorded_profile_absolute_paths", "(auto-learned)", "(nginx + rendered-redis startup)", false, passed, detail)
	})
